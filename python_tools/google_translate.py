...
# 微信公众号：爱上资讯吧 
# In advance:this page don't original
import os,sys,time
import traceback
import subprocess
import googletrans
...
def translate_word(text,src, dest):
    try:
        tr = g_trans.translate(text,dest,src)
    except:
        traceback.print_exc()
        msg = '\t<b>Google翻译异常，请稍后再试 请检查你的网络是否在线</b>'
        return msg
    print(tr.text)
    return tr.text
...
class Window(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        mainLayout.addWidget(wordLabel, 3, 0)
        mainLayout.addWidget(self.textbox, 3, 1)
        mainLayout.addWidget(self.fileComboBox, 2, 1)
        mainLayout.addWidget(self.browseButton, 2, 2)
        mainLayout.addWidget(self.logPlainText, 4, 0, 1, 3)
        ...
        self.setWindowTitle("Translate tool base of google")
        self.resize(500, 200)
    ...
    def translate(self):
        ...
        if words!="":
          ...
          if not lang_dst:
              self.logger.show.emit('无效的目标语言:{}'.format(lang_dst_select))
              return
          print(lang_src, lang_dst)
          wordp=translate_word(words,lang_src,lang_dst)
          self.logger.show.emit(wordp)
          
          
        if not fileName:
          if words=="":
            self.logger.show.emit('请先选择要翻译的Word文档 或者单词')
          return
